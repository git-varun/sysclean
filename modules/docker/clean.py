...
def plan():
    ...
    out = run_cmd(["docker", "images", "-f", "dangling=true", "-q"])
    ...
    out = run_cmd(["docker", "ps", "-a", "-f", "status=exited", "-q"])
...
def execute():
    ...
    for t in targets:
        if t["id"] == "docker_dangling_images" and t["items"]:
            try:
                subprocess.check_call(["docker", "rmi"] + t["items"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            ...
            try:
                subprocess.check_call(["docker", "rm"] + t["items"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            except subprocess.CalledProcessError as e:
                status = "partial_failure"
